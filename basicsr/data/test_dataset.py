...
class TestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.opt = opt
        self.dir_ldr = os.path.join(opt['dataroot'], 'LDR/')
        self.dir_hdr = os.path.join(opt['dataroot'], 'HDR/')
        self.dir_voxel = os.path.join(opt['dataroot'], 'event/')

        self.ldr_paths = sorted(make_dataset(self.dir_ldr, opt.max_dataset_size))
        self.im_paths = sorted(make_dataset(self.dir_voxel, opt.max_dataset_size))
    ...
    def __getitem__(self, index):
        ...
        return {'input_ldr_y': data_ldr_y, 'input_ldr_u': data_ldr_u, 'input_ldr_v': data_ldr_v,
                'input_ldr_rgb': data_ldr_rgb, 'input_im': im_tensor, 'paths': ldr_path,
