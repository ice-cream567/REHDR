...
class BaseModel():
    ...
    #     """Reduce a single loss value in distributed training.
    #
    #     In distributed training, it averages the loss among different GPUs.
    #
    #     Args:
    #         loss (Tensor): A single loss value (PyTorch tensor).
    #
    #     Returns:
    #         float: The reduced and averaged loss value as a scalar.
    #     """
    #     with torch.no_grad():
    #         if self.opt['dist']:
    #             # 在分布式训练中对损失值进行同步和规约
    #             torch.distributed.reduce(loss, dst=0)  # 将所有 GPU 的损失值规约到主进程 (rank 0)
    #             if self.opt['rank'] == 0:
    #                 loss /= self.opt['world_size']  # 在主进程上对损失值求平均
    #
    #         # 返回标量值（所有 GPU 上的损失值保持一致）
    #         return loss.item()